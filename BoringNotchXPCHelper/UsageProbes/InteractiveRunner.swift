...
public struct InteractiveRunner: Sendable {
    ...
    public struct Options: Sendable {
        ...
        /// Substrings (matched against the ANSI-normalized buffer) that mark the
        /// command's final output. Until at least one appears, the idle-exit is
        /// suppressed so a TUI that renders "loading…" then goes silent during a
        /// network fetch isn't cut off before it draws the real data.
        public var completionMarkers: [String]
        ...
        public init(
            ...
            self.completionMarkers = completionMarkers
        }
    }
    ...
    private func captureOutput(
        ...
        // Prompt keys are matched case-insensitively against a normalized view of
        // the buffer (ANSI/escape sequences replaced by spaces). CLIs like Claude
        // render each word at a fixed column via cursor-move escapes, so words in
        // the raw byte stream are separated by escapes rather than spaces — a raw
        // substring match on a multi-word prompt would never fire.
        let promptResponses = options.autoResponses.map {
            (key: $0.key.lowercased(), response: Data($0.value.utf8))
        }
        var respondedPrompts = Set<String>()

        let completionMarkers = options.completionMarkers.map { $0.lowercased() }
        var completionSeen = completionMarkers.isEmpty
        ...
        while Date() < deadline {
            ...
            let normalized = normalizedForMatching(buffer)
            for item in promptResponses where !respondedPrompts.contains(item.key) {
                if normalized.contains(item.key) {
                    try? handle.write(contentsOf: item.response)
                    respondedPrompts.insert(item.key)
                    lastMeaningfulDataTime = Date()  // Reset idle timer after responding
                }
            }

            // Note once the command's final output has appeared.
            if !completionSeen, completionMarkers.contains(where: { normalized.contains($0) }) {
                completionSeen = true
            }
            ...
            // idleTimeout — but only after any required completion marker appeared,
            // so a silent "loading…" phase doesn't cut the read short.
            if completionSeen
                && hasMeaningfulContent(buffer)
    ...
    /// Produces a lowercased, whitespace-collapsed view of the buffer with every
    /// ANSI/control escape sequence replaced by a space, so multi-word prompt keys
    /// match even when the CLI positions each word with cursor-move escapes.
    private func normalizedForMatching(_ data: Data) -> String {
        guard let text = String(data: data, encoding: .utf8) else { return "" }

        // Replace CSI sequences (incl. cursor moves like ESC[25G) with a space.
